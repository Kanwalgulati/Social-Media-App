  ...
  try {
    ...
    if (!caption) {
      return res.status(401).json({
        success: false,
        message: "Please Enter Valid Caption",
      });
    }
    if (!post) {
      return res.status(404).json({
...
exports.deleteComment = async (req, res) => {
  try {
    ...
    //@ IF its user then he can delete any comment he wants
    ...
    if (post.owner.toString() === req.user._id.toString()) {
      ...
      if (targetIndex != -1) {
        console.log("post.comm", post.comments);
        post.comments.splice(targetIndex, 1);
        console.log("post.comm 22", post.comments);
      }
